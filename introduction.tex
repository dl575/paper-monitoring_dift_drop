...
paper, we explore a new trade-off dimension for run-time monitoring designs. We
propose to enable low overhead monitors without sacrificing flexibility by
using partial monitoring. This instead allows a trade-off between overheads and
...
These monitors show baseline overheads of 4-16x without null metadata filtering
and 1.1-4.3x after null metadata filtering.  Our results show that with 1.5x
overhead, coverage of 43-82\% on average can still be achieved and that
overhead and coverage can be varied to create a trade-off.
