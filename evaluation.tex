...
DIFT sees an average of 10\% overhead with a maximum overhead of 31\% with null
metadata filtering. This is due, in part, to the fact
...
executions from 2-6x with an average of 4x. Overheads for LS (not pictured) are
1.03-1.72x after null metadata filtering and the average
overhead observed is 1.33x for the SPLASH-2 benchmarks.
These overheads are similar to the overheads seen by previous multi-core
...
policy is more appropriate. On the other hand, if occasional slowdowns are
acceptable, then source-only dropping can provide better coverage on average.
